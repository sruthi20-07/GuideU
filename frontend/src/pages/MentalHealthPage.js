...
export default function MentalHealthPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const ref = doc(db, "mentalHealth", auth.currentUser.uid);
...
if (diffHours < 24) {
  ...
  const saveCheckIn = async () => {
    ...
    await setDoc(doc(db, "mentalHealth", auth.currentUser.uid), data);
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        <div style={styles.qBlock}>
          ...
          <input style={styles.input} type="number" name="sleepHours"
            disabled={!canEdit} value={form.sleepHours} onChange={handleChange} />
        </div>

        <div style={styles.qBlock}>
          <p>How many hours did you study?</p>
          <input style={styles.input} type="number" name="studyHours"
            disabled={!canEdit} value={form.studyHours} onChange={handleChange} />
        </div>
